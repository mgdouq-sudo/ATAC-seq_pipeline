...
```{r}
resOrdered <- results_cDC1[order(results_cDC1$padj),]
...
```{r}
sig_cDC1 <- deseq2_res %>% relocate(gene)
sig_cDC1$significant <- "No"  # default: not significant
sig_cDC1$significant[deseq2_res$padj < 0.05] <- "Yes"
# Direction = based on padj < 0.05 only (no fold-change filter)
sig_cDC1$direction <- "NotSig"
sig_cDC1$direction[deseq2_res$padj < 0.05 & deseq2_res$log2FoldChange > 0] <- "Up"
sig_cDC1$direction[deseq2_res$padj < 0.05 & deseq2_res$log2FoldChange < 0] <- "Down"
...
cat("cDC1 significant genes:", sum(sig_cDC1$significant == "Yes"), "\n")
```
