...
```{r}
atac_data <- read_csv("cDC1_diffbind_ALL_annotated_unique_genes.csv") %>% dplyr::select(gene, log2FoldChange_ATAC, pvalue_ATAC = pvalue, peak_type)
```

```{r}
rna_data <- read_csv("cDC1_DESeq2_significant.csv") %>% dplyr::select(gene, log2FoldChange_RNA = log2FoldChange, padj_RNA = padj)
```
```{r}
...
```
```{r}
ggsave("cDC1_ATAC_RNA_correlation.png", width = 8, height = 6)
```
